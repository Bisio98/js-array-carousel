...
const arrowDown = document.querySelector('.up');
arrowDown.addEventListener('click', function() {
    
    bigPhoto[onImage].classList.remove('show');
    smallPhoto[onImage].classList.remove('active');

    if ( onImage > 0 ) {
        onImage--;
    } else {
        onImage = imgArray.length - 1;
    }
...
const arrowUp = document.querySelector('.down');
arrowUp.addEventListener('click', function() {

    bigPhoto[onImage].classList.remove('show');
    smallPhoto[onImage].classList.remove('active');

    if( onImage < imgArray.length - 1 ) {
        onImage++;
    } else {
        onImage = 0;
    }
